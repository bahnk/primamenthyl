...
def resolve_paths() -> tuple[Path, Path]:
    ...
    plots_dir = repo_root / "output" / "plots" / "five_prime_end_motif_distribution"
    ...
    try:
        for sample in fetch_samples(connection):
            rows = connection.execute(
                ...
                WHERE sample = ? AND side = 'five_prime'
            ...
            axis.set_title(f"5' End Motif Distribution: {sample}")
            axis.set_xlabel("Motif count")
            axis.set_ylabel("Motif")

            output_path = plots_dir / f"{sample}.five_prime_end_motif_distribution.pdf"
