...
def resolve_paths() -> tuple[Path, Path]:
    ...
    plots_dir = repo_root / "output" / "plots" / "start_position_distribution"
...
def fetch_samples(connection: duckdb.DuckDBPyConnection) -> list[str]:
    return [
        ...
        for row in connection.execute(
            "SELECT DISTINCT sample FROM start_position_distribution ORDER BY sample"
        ).fetchall()
    ]
    ...
    try:
        for sample in fetch_samples(connection):
            rows = connection.execute(
                """
                SELECT
                    floor(start_position / ?) * ? AS start_bin,
            ...
            start_positions = [row[0] for row in rows]
            ...
            axis.set_title(f"Start Position Distribution: {sample}")
            axis.set_xlabel(f"Start position bin ({bin_size} bp)")
            axis.set_ylabel("Record count")

            output_path = plots_dir / f"{sample}.start_position_distribution.pdf"
